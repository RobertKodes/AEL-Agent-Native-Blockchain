...
test('replica keeps state, manifest, and website alive after its origin stops',async t=>{
  ...
  const state=await fetch(`http://127.0.0.1:${port}/v1/state`).then(r=>r.json()),manifest=await fetch(`http://127.0.0.1:${port}/v1/manifest`).then(r=>r.json()),home=await fetch(`http://127.0.0.1:${port}/`).then(r=>r.text()),article=await fetch(`http://127.0.0.1:${port}/article`).then(r=>r.text()),releaseHash=await fetch(`http://127.0.0.1:${port}/downloads/release.sha256`).then(r=>r.text()),sbom=await fetch(`http://127.0.0.1:${port}/downloads/sbom.spdx.json`).then(r=>r.json());
  assert.equal(state.accounts.survivor.native,7);assert.equal(manifest.role,'VERIFIED_READ_ONLY_REPLICA');assert.equal(manifest.writes.accepted,false);assert.match(manifest.downloads.sbom,/sbom\.spdx\.json$/);assert.match(home,/<title>AEL \/\/ the agent-native network<\/title>/);assert.match(article,/<title>Launch Article \/\/ AEL<\/title>/);assert.match(releaseHash,/^[a-f0-9]{64}/);assert.equal(sbom.spdxVersion,'SPDX-2.3');assert.equal((await fetch(`http://127.0.0.1:${port}/health`).then(r=>r.json())).survivesUpstreamOutage,true);
});
