...
test('wallet extension exposes only consent-gated AEL provider methods',()=>{
  ...
  assert.match(background,/senderOrigin!==message\.origin/);assert.match(background,/connections\[senderOrigin\]\?\.actorId===vault\?\.actorId/);assert.match(background,/\/v1\/token/);assert.match(popup,/permissions\.request/);assert.match(popup,/revoke-sites/);assert.deepEqual(manifest.optional_host_permissions,['https://*/*']);
});
...
test('browser packages are deterministic in structure and declare honest signing boundaries',()=>{
  ...
  assert.equal(chromium.manifest_version,3);assert.equal(chromium.version,'0.6.0');assert.equal(chromium.background.service_worker,'background.js');assert.equal(chromium.content_scripts[0].run_at,'document_start');
  assert.deepEqual(firefox.background.scripts,['background.js']);assert.equal(firefox.browser_specific_settings.gecko.id,'wallet@ael.network');
  assert.equal(release.distribution.chrome,'REQUIRES_CHROME_WEB_STORE_REVIEW');assert.equal(release.distribution.firefox,'REQUIRES_MOZILLA_SIGNATURE');assert.equal(release.distribution.safari,'REQUIRES_APPLE_DEVELOPER_SIGNING');assert.equal(release.privateKeyHandling,'LOCAL_ENCRYPTED_VAULT_ONLY');
});
