...
const implementationRoots=['package.json','README.md','LICENSE','CONTRIBUTING.md','SECURITY.md','CODE_OF_CONDUCT.md','.github','ci','Dockerfile','Dockerfile.actor','Dockerfile.node','Dockerfile.consensus','compose.yaml','compose.node.yaml','compose.mirror.yaml','compose.agent-node.yaml','compose.bft.yaml','.dockerignore','.railwayignore','.gitignore','agent-sdk-py','docs','ops','scripts','skills','src','test','test-vectors','wallet-extension','web'];
...
const implementation=implementationRoots.flatMap(walk).filter(path=>path!=='ops/railway-deployment.json'&&(!tracked||tracked.has(path))).sort();
