...
const roots = ['package.json','README.md','LICENSE','CONTRIBUTING.md','SECURITY.md','CODE_OF_CONDUCT.md','.github','Dockerfile','Dockerfile.actor','Dockerfile.node','Dockerfile.consensus','compose.yaml','compose.node.yaml','compose.mirror.yaml','compose.agent-node.yaml','compose.bft.yaml','.dockerignore','.railwayignore','.gitignore','docs','ops','scripts','skills','src','web','wallet-extension','agent-sdk-py','test','test-vectors','dist/test-vectors','dist/phase-gates.json','dist/audit-manifest.json'];
const files = roots.flatMap(function walk(path) { return statSync(path).isDirectory() ? readdirSync(path).sort().flatMap(name => walk(`${path}/${name}`)) : [path]; }).filter(path=>path!=='ops/railway-deployment.json').sort();
