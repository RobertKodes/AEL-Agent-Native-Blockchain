...
if (!once) {
  ...
  createServer((request, response) => {
    ...
    if(path==='/v1/manifest'||path==='/.well-known/ael.json')return send(200,{schema:'AEL-REPLICA-MANIFEST/1',chainId:'ael-devnet-1',origin,role:'VERIFIED_READ_ONLY_REPLICA',upstreams:syncSources,state:`${origin}/v1/state`,checkpoint:`${origin}/v1/state/checkpoint`,certificate:`${origin}/v1/consensus/certificates/latest`,validatorSet:`${origin}/v1/consensus/validator-set`,website:`${origin}/`,writes:{accepted:false,discovery:'Use another healthy origin; verify its finality certificate.'}});
    const pages={'/':'index.html','/about':'about.html','/agents':'agents.html','/operators':'operators.html','/network':'network.html','/token':'token.html','/developers':'developers.html','/whitepaper':'whitepaper.html','/portal.js':'portal.js','/styles.css':'styles.css','/llms.txt':'llms.txt','/openapi.json':'openapi.json'};
    if(pages[path]){try{const data=readFileSync(new URL(`../web/${pages[path]}`,import.meta.url)),types={'.html':'text/html','.js':'text/javascript','.css':'text/css','.txt':'text/plain','.json':'application/json'};response.writeHead(200,{'content-type':`${types[extname(pages[path])]??'application/octet-stream'}; charset=utf-8`,'x-ael-replica':'read-only'});return response.end(data);}catch{}}
    if(path==='/downloads/release'){try{response.writeHead(200,{'content-type':'application/gzip','content-disposition':'attachment; filename="ael-local-devnet.tar.gz"','x-ael-replica':'read-only'});return response.end(readFileSync(new URL('../dist/ael-local-devnet.tar.gz',import.meta.url)));}catch{}}
